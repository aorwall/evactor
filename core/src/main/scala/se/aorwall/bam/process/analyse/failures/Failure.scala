...
class Failure (name: String, eventName: Option[String], maxOccurrences: Long, window: Option[WindowConf])
  extends ProcessorConfiguration(name) {

    def getProcessor = window match {
      case Some(length: LengthWindowConf) => new FailureAnalyser(name, eventName, maxOccurrences) with LengthWindow {override val noOfRequests = length.noOfRequests}
      case Some(time: TimeWindowConf) => new FailureAnalyser(name, eventName, maxOccurrences) with TimeWindow {override val timeframe = time.timeframe}
      case None => new FailureAnalyser(name, eventName, maxOccurrences)
    }
}