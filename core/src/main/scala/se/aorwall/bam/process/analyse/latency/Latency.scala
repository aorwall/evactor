...
class Latency (name: String, eventName: Option[String], maxLatency: Long, window: Option[WindowConf])
  extends ProcessorConfiguration(name) {

   def getProcessor = window match {
      case Some(length: LengthWindowConf) => new LatencyAnalyser(name, eventName, maxLatency) with LengthWindow {override val noOfRequests = length.noOfRequests}
      case Some(time: TimeWindowConf) => new LatencyAnalyser(name, eventName, maxLatency) with TimeWindow {override val timeframe = time.timeframe}
